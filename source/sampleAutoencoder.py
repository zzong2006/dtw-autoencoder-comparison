...
rbmObject01 = BBRBM(n_visible=n_input, n_hidden=n_hidden, learning_rate=learning_rate, momentum=0.95, use_tqdm=False)
rbmObject02 = BBRBM(n_visible=n_hidden, n_hidden =n_input, learning_rate= learning_rate, momentum=0.95, use_tqdm= False)
...
W_encode = tf.get_variable(shape = [n_input, n_hidden], name="W_encode"
                            , initializer=tf.contrib.layers.xavier_initializer())
b_encode = tf.Variable(tf.random_normal([n_hidden]), name="b_encode")

saver = tf.train.Saver({'RBM_vr1' + '_w': W_encode,
                        'RBM_vr1' + '_h': b_encode})
saver.restore(sess, './out/RBM_data1')

# saver = tf.train.Saver({W_encode: 'rbmw1'}, {'rbmhb1': b_encode})
# saver.restore(sess, './out/rbmw1.chp')
...
W_decode = tf.get_variable(shape = [n_hidden, n_input], name="W_decode"
                            , initializer=tf.contrib.layers.xavier_initializer())
b_decode = tf.Variable(tf.random_normal([n_input]))

saver = tf.train.Saver({'RBM_vr2' + '_w': W_decode,
                        'RBM_vr2' + '_h': b_decode})
