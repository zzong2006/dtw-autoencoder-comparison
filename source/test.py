...
MAX_NUM = 300000                            # 단일 데이터가 가질 수 있는 최대값
MIN_NUM = 0                                 # 단일 데이터가 가질 수 있는 최소값
VAR_MEAN = 0.0                              # 변위로 사용할 정규분포의 평균
VAR_SIGMA = (MAX_NUM/numberOfWindow) * 0.25  # 변위로 사용할 정규분포의 표준편차

def get_truncated_normal(mean=0, sd=1, low=0, upp=10):
    return truncnorm((low - mean) / sd, (upp - mean) / sd, loc=mean, scale=sd)

def generate_data_to_file(fileName='data.csv',GEN_DATA_PER_WINDOW=1000, GEN_WINDOW_QUANTITY=200):
    # 스트림 데이터 생성 시작
    ...
    for VAR_MEAN in means:
        stream_data = np.append(stream_data, get_truncated_normal(VAR_MEAN, VAR_SIGMA, MIN_NUM, MAX_NUM).rvs(GEN_DATA_PER_WINDOW))
    ...
    stream_data = list(map(int,  stream_data))
    print(VAR_SIGMA)
