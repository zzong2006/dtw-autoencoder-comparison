# 이 소스의 출처: https://github.com/Cospel/rbm-ae-tf/blob/master/rbm.py
...
class RBM(object):
    def __init__(self, n_input, n_hidden, layer_names, alpha=1.0, transfer_function=tf.nn.sigmoid):
        ...
        self.h1 = self.sample_prob(tf.matmul(self.v1, self.rbm_w) + self.rbm_hb)
        ...
        self.update_w = self.rbm_w + alpha * (self.w_positive_grad - self.w_negative_grad)\
    ...
    def _initialize_weights(self):
        ...
        all_weights = dict()
        all_weights['w'] = tf.get_variable(shape = [self.n_input, self.n_hidden], name=self.layer_names[0]
                           , initializer=tf.contrib.layers.xavier_initializer())
        # tf.Variable(tf.random_normal([self.n_input, self.n_hidden], stddev=0.01, dtype=tf.float32),name=self.layer_names[0])
        all_weights['vb'] = tf.Variable(tf.random_normal([self.n_input], dtype=tf.float32), name=self.layer_names[1])
        all_weights['hb'] = tf.Variable(tf.random_normal([self.n_hidden], dtype=tf.float32), name=self.layer_names[2])
        return all_weights
    ...
    def save_weights(self, path):
        ...
        print('weight and bias saved in :' + save_path)
